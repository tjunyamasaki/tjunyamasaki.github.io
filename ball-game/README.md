...
Choose **Draw your cell** in the menu or room lobby. Paint with a mouse or finger, choose seven colors and three brush sizes, erase, undo (up to 24 edits), or clear. Use drawing saves it on this device and applies it to every fragment, including after respawn. Cancel leaves the current drawing unchanged. Transparent areas retain your chosen cell color.

The 64×64 indexed drawing is a validated 4 KiB string, never an uploaded image or executable URL. Each player's texture is cached on receipt, clipped to the cell, and shared on reliable control messages when it changes and when new players join. It is excluded from frequent gameplay snapshots.

## Multiplayer

Host friends to open a lobby, copy the invitation (or share the five-character code), and have friends choose Join room. The roster updates as people connect; the host can adjust bots and press Start game when everyone is there. All simulation and gameplay actions remain frozen until the host starts. Maximum: 8 humans plus 16 bots. Guests may join an ongoing game. Host keeps their tab foregrounded; hiding it pauses simulation. Guest departure removes their cells; a disconnected player can rejoin as a fresh player. There is no host migration or saved match state.
...
Engine tests cover eating thresholds, split/merge including convergence, ejection, viruses, seeded food, movement sanitization, bounds, bots, capacity, respawn, frame size, and a one-minute simulation. Transport tests connect two production network endpoints through in-memory signaling/data-channel fakes, verify identity binding, movement/actions, state/food exchange, and cleanup. Lobby tests verify frozen simulation, roster updates at unchanged simulation time, and host-only start. Drawing tests cover bounded data, brush/eraser strokes, sharing and late joiners. Those tests do not validate public Firebase permissions, real ICE negotiation or phone rendering/performance.
...
`.github/workflows/deploy-pages.yml` runs tests, writes the existing Firebase config from repository secrets, includes this directory in the Pages artifact, and deploys on pushes to `master` and `feat/ball-game`. Either deployment branch publishes the production Pages site. GitHub's `github-pages` environment must permit this branch. No changes to other games' networking or rules.
