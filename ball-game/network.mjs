...
export const PROTOCOL='ball-game-v1';
export function createNetwork({world,onFrame,onFood,onReady,onStatus,onLeave,signaling,PeerConnection=globalThis.RTCPeerConnection}) {
  ...
  async function host() {
    await initialize();hosting=true;code=await signal.createRoom(`${PROTOCOL}:${id}`);
  ...
  async function join(room,name,hue) {
    ...
    pc.ondatachannel=e=>{
      ...
      if(ch.label===PROTOCOL){guest.control=ch;ch.onopen=()=>send(ch,{type:'hello',protocol:PROTOCOL,hue});ch.onclose=()=>fail('The host left the arena.');}
  ...
  return {host,join,broadcast,input,action,ping,pause,stop};
}
