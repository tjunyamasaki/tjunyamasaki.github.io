...
test('T31 v1 migration preserves items without moving the live clock',()=>{
  ...
  for(const doc of [normal, full, ...boundaries.saves]){
    ...
    const resumed=World.restore(kept.save.world);
    assert.equal(resumed.clock,CLOCK_V1);
    assert.equal(resumed.time,doc.world.time);
    assert.equal(phaseAt(resumed.time),phaseAt(doc.world.time));
    assert.throws(()=>World.restore(mapped.save.world),/not a Hollowstead expedition/);
  ...
  const resumed=World.restore(migrateV1Save(normal,{remapTime:false}).save.world);
  ...
  assert.equal(resumed.nodes.find(node=>node.id==='n0').ready,755);

  const fullWorld=World.restore(migrateV1Save(full,{remapTime:false}).save.world);
...
test('T32 corrupt migration keeps the original, and overflow can be withdrawn',()=>{
  ...
  assert.equal(continued.save.world.clock,CLOCK_V1);
  ...
  assert.equal(future.code,'clock');

  const full=loadFixture('v1-full-storage.json');
  const world=World.restore(migrateV1Save(full,{remapTime:false}).save.world);
...
test('T33 save snapshots drop live activity and network snapshots keep it',()=>{
  ...
  assert.equal(network.clock,CLOCK_V1);
  assert.equal(saved.clock,CLOCK_V1);
