...
const TOLLED=100-BELL.damage;

test('Mourning Bell waits for its wave, hits each hostile once, and preserves allies',()=>{
  const {world,p}=setup();
  const near=foe(world,'near',1),edge=foe(world,'edge',BELL.radius),outside=foe(world,'outside',BELL.radius+.2);
...
test('network/save round trips preserve travelling rings and their already-hit targets',()=>{
  ...
  world.attack(p);advance(world,.5);assert.equal(near.hp,TOLLED);assert.equal(far.hp,100);
  ...
  advance(guest,.6);
  assert.equal(guest.enemies.find(e=>e.id==='near').hp,TOLLED);
...
test('Gravecraft damage grows with the wanderer level, like every other weapon',()=>{
  const {world,p}=setup();p.level=6;const near=foe(world,'near',1);
  world.attack(p);for(let i=0;i<20;i++)advance(world,.05);
  assert.equal(near.hp,100-Math.round(BELL.damage*1.2));
});
