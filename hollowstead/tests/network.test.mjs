...
test('host and guest exchange lobby, actions, chunked state, pause and cleanup',async()=>{
 ...
 try{
  ...
  guest.action({type:'craft',recipe:'axe'});await flush();assert.equal(world.player(id).equipment.axe,70);assert.equal(world.player('host').equipment.axe,undefined);
  for(let i=0;i<120;i++)world.buildings.push(world.structure('wall',20+i%10,20+Math.floor(i/10)));
  assert.ok(JSON.stringify(world.snapshot()).length>12000);host.broadcast();await flush();assert.equal(frame.buildings.length,121);assert.equal(frame.players.find(p=>p.id===id).equipment.axe,70);
...
test('a reconnect using the same tab identity keeps equipment and inventory',async()=>{
 ...
 try{await host.host();await guest.join('ABCDE','Friend','moss');await flush();world.player(identity).inventory.ember=9;world.player(identity).equipment.sword=120;await guest.stop();await flush();assert.equal(world.player(identity).online,false);guest=createNetwork({...opts,identity});await guest.join('ABCDE','Friend','moss');await flush();assert.equal(world.players.length,2);assert.equal(world.player(identity).equipment.sword,120);assert.equal(world.player(identity).inventory.ember,9);}finally{await guest.stop();await host.stop();}
});
