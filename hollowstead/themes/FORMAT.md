...
| `variants` | Optional list of other sprite keys. Each entity picks one by hashing its id, so it keeps the same look for its whole life. Repeat a key to weight it. |
| `icon` | Optional single-image URL used for inventory, recipe and build-list icons instead of the (possibly animated) sheet. |
...
All supplied SVGs are original editable vector artwork, generated by `tools/art/build.py` (Python, `cairosvg` + `pillow`). The generator fits every drawing into the footprint the sprite should occupy in its 512 x 768 cell and rescales line widths so every sprite has the same ink weight on screen. Edit a drawing in `tools/art/*.py`, then run `python3 tools/art/build.py` (or `--only key1,key2 --preview out.png`).

Sheet layouts in the default theme:

| Sprites | Grid | Frames |
| --- | --- | --- |
| `ember`, `moss`, `vesper`, `cinder` | 8 x 2 | 0-1 idle, 2-5 walk, 6-8 attack, 9-11 gather, 12 dash, 13 down |
| `crawler`, `wraith`, `brute`, `king` | 4 x 2 | 0-3 walk, 4-6 attack (wind-up, strike, recover), 7 idle |
| `fire`, `hearth`, `pot`, `lantern`, `ward` | 4 x 1 | looping idle |
| `tree` / `tree-dead`, `rock` / `rock-rune` | 1 x 1 | variants of the same node |

The previous artwork is kept for reference in `../legacy/`; nothing in the game loads it. The Three.js renderer reuses the repository's existing vendored Three.js and license under `hushlight/vendor/`; no runtime asset CDN or new game engine is introduced.
